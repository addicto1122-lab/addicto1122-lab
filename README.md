...
- HTML5, CSS, JavaScript 기반 화면 구현
- React, React Router, Hooks 활용
- jQuery, AJAX, Bootstrap 활용
...
- 💻 **Development** · [Development Portfolio](https://lying-sauce-1dc.notion.site/3e1a87937d55813d8edafd73f05f27ea?source=copy_link)
- 🖥️ **Infrastructure** · [Infrastructure Portfolio](https://lying-sauce-1dc.notion.site/3e2a87937d55818eb0dbf06167cf1578?source=copy_link)

---

## 🚀 Main Project

### [MeetBack｜모두가 안전하게 귀가할 수 있는 모임 장소 추천 서비스](https://github.com/addicto1122-lab/meetback)

> 여러 참가자의 귀가 시간과 막차 정보를 분석해  
> 모두가 안전하게 귀가할 수 있는 모임 장소를 추천하는 서비스입니다.

**프로젝트 기간:** 2026.08.18 ~ 2026.09.02  
**수행 인원:** 4명  
**담당 역할:** 귀가·추천 로직, 외부 API 연동, 지도 시각화, API 호출 최적화

#### 주요 기능

- Kakao Local API 기반 장소 검색
- ODsay API 기반 대중교통 경로 및 막차 정보 조회
- 참가자별 안전 귀가 가능 여부 계산
- 평균 귀가시간과 편차를 반영한 장소 추천
...
<details>
<summary><strong>🔧 주요 트러블슈팅 펼쳐보기</strong></summary>

<br>

#### 1. ODsay 연속 요청 오류와 긴 대기 시간 개선
...
- API 요청 안정성을 유지하면서 전체 경로 계산 시간을 단축했습니다.

---

#### 2. 일일 API 호출 한도 감소 대응
...
#### 3. 주소 형식에 따른 좌표 조회 오류 개선

**문제**

- 사용자가 입력한 주소를 그대로 Kakao Local API에 전달할 경우 일부 주소에서 실제 위치와 다른 x·y 좌표가 반환되는 문제가 발생했습니다.
- 잘못된 좌표는 이후 거리 계산과 ODsay 경로 조회 결과에도 영향을 줄 수 있었습니다.

**해결**

- 사용자 주소를 도로명 주소 기준으로 조회하도록 변경했습니다.
- 도로명 주소를 Kakao Local API에 전달해 반환받은 x·y 좌표를 거리 계산과 경로 조회에 사용했습니다.

**결과**

- 주소 형식에 따라 잘못된 좌표가 반환되는 문제를 줄이고 이후 거리·경로 계산의 정확도를 개선했습니다.

---

#### 4. 700m 미만 경로의 ODsay 오류 처리
...
- **리눅스마스터 2급 1차 합격** — KAIT, 2026.08
- **네트워크관리사 2급** — 한국정보통신자격협회, 2025.08
- **PC정비사 2급** — 한국정보통신자격협회, 2025.01
- **전자계산기기능사** — 한국산업인력공단, 2022.12
- **전자기능사** — 한국산업인력공단, 2022.07
